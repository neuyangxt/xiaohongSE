...
static inline esp_err_t es8311_write_reg(es8311_handle_t dev, uint8_t reg_addr, uint8_t data)
{
    ...
    return i2c_master_transmit(es->dev_handle, write_buf, sizeof(write_buf), pdMS_TO_TICKS(1000));
}

static inline esp_err_t es8311_read_reg(es8311_handle_t dev, uint8_t reg_addr, uint8_t *reg_value)
{
    es8311_dev_t *es = (es8311_dev_t *) dev;
    return i2c_master_transmit_receive(es->dev_handle, &reg_addr, 1, reg_value, 1, pdMS_TO_TICKS(1000));
}
...
{
    if (bus_handle == NULL) {
        ESP_LOGE(TAG, "invalid i2c master bus handle");
        return NULL;
    }

    es8311_dev_t *es = calloc(1, sizeof(es8311_dev_t));
    if (es == NULL) {
        ESP_LOGE(TAG, "no memory for es8311_dev_t");
        return NULL;
    }
    ...
    esp_err_t ret = i2c_master_bus_add_device(bus_handle, &dev_cfg, &es->dev_handle);
    if (ret != ESP_OK) {
