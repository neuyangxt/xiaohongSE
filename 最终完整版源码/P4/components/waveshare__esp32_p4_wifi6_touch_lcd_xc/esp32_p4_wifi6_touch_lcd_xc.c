...
/* S71D: reuse OpenHarmony shared I2C bus on GPIO7/GPIO8.
 * Audio BSP must not create a second I2C master bus after ST7123 touch has been registered.
 */
extern esp_err_t OhosBoardI2cGetSharedBus(i2c_master_bus_handle_t *out_bus);
...
esp_err_t bsp_i2c_init(void)
{
    ...
    if (i2c_initialized) {
        ESP_LOGI(TAG, "S71D bsp_i2c_init already initialized bus=%p", i2c_handle);
        return ESP_OK;
    }

    i2c_master_bus_handle_t shared_bus = NULL;
    esp_err_t shared_ret = OhosBoardI2cGetSharedBus(&shared_bus);
    if (shared_ret == ESP_OK && shared_bus != NULL) {
        i2c_handle = shared_bus;
        i2c_initialized = true;
        ESP_LOGI(TAG, "S71D bsp_i2c_init reuse OHOS shared I2C bus=%p", i2c_handle);
        return ESP_OK;
    }

    ESP_LOGW(TAG, "S71D OHOS shared I2C unavailable ret=%s bus=%p, fallback to BSP i2c_new_master_bus",
             esp_err_to_name(shared_ret), shared_bus);
